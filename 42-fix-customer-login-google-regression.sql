-- ============================================================
-- Fix: customer_login_google() lost its staff-account guard
-- ============================================================
-- Run this on: https://edunsrtcdhnpbsipalhc.supabase.co
--
-- BUG FOUND: 19-block-staff-customer-login.sql added a check to
-- customer_login_google() so a staff member's whitelisted Google
-- account could never get a customer card ("raise exception
-- staff_account" if the signed-in email matches staff_users.google_email).
-- 23-student-status.sql later redefined the same function (to add the
-- is_student column to its return shape) but was written from the
-- pre-19 version — it silently dropped that guard. This restores it
-- on top of 23's current (is_student-including) shape.
--
-- ALSO HARDENED: the original find-or-create was a plain SELECT then
-- INSERT with no protection against two near-simultaneous calls (e.g.
-- a slow network prompting a retry) both missing the SELECT and then
-- both trying to INSERT the same email/auth_uid — the second one
-- would hit customers_email_unique or customers_auth_uid_unique and
-- fail with a raw constraint-violation error, which is exactly the
-- shape of a generic "Google sign-in failed" report with no clearer
-- cause. Switched the insert to ON CONFLICT DO NOTHING + a re-select,
-- so a losing race just finds the row the other call created instead
-- of erroring.
-- ============================================================
...
begin
  ...
  if found then
    ...
    -- Bare ON CONFLICT DO NOTHING (no target list) absorbs a violation
    -- on either unique index (auth_uid or email) — a losing race just
    -- means the winner's row gets picked up by the re-select below,
    -- instead of this raising a raw constraint-violation error.
